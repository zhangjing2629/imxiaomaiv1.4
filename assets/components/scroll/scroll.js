...
(function(angular) {
   ...
   *      momentum: false} }}"">...</div>
   */
  angular.module('xiaomaiApp').directive('scrollable',
        ...
        link: function(scope, element, attrs) {
          ...
          var handleScrollEnd = function() {
            $timeout(function() {
              ...
              if (angular.isDefined(scope.isMaxY)) {
                scope.isMaxY = (iScrollInstance.y <=
                  iScrollInstance.maxScrollY);
              }
              ...
              if (angular.isDefined(scope.isMaxX)) {
                scope.isMaxX = (iScrollInstance.x >=
                  iScrollInstance.maxScrollX);
              }
          ...
          var scrollToPageX = function(pageX) {
            if (angular.isDefined(iScrollInstance.pages) &&
              iScrollInstance.pages.length !== 0 && angular.isDefined(
                pageX)) {
          ...
          var _init = function() {
            ...
            $window.addEventListener(orientationEvent, function() {
              if (scr.width !== screen.width || scr.height !==
                screen.height) {
                scr = {
                  'width': screen.width,
          ...
          attrs.$observe('parameters', function(val) {
            ...
            scope.iscrollParameters.click = angular.isDefined(val.click) ?
              val.click : true;
          });
          ...
          scope.$watch(function() {
              return scroller[0].offsetHeight;
            },
            _refresh
          );
          scope.$watch(function() {
              return scroller[0].offsetWidth;
            },
            _refresh
          );
